...
**Your answer here**: **routing** refers to how a server handles different HTTP methods and url paths. Conditional statements such as `if-else` statements are used to handle multiple routes. After every conditional it is important to use a `return` statement because it prevents the code from reaching another conditional block and triggering the `.end()` method a second time, causing an `Error write after end` error. This means that we tried to send two responses per request.
